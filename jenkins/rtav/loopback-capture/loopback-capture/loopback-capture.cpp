...
HRESULT LoopbackCapture(
    ...
    *pnFrames = 0;
	...
	UINT32 silentAndDiscontinuityCount = 0;
    bool bDone = false;
    ...
    for (UINT32 nPasses = 0; !bDone; nPasses++) {
        ...
        for (
			...
			if (bQuit && dwFlags == 0) {
				LOG(L"%s", L"Quiting on receiving first sound data packet");
				return hr;
			}
			...
			else if (dwFlags == AUDCLNT_BUFFERFLAGS_SILENT + AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY) {
				silentAndDiscontinuityCount++;
			}
			else if (0 != dwFlags) {
                LOG(L"IAudioCaptureClient::GetBuffer set flags to 0x%08x on pass %u after %u frames", dwFlags, nPasses, *pnFrames);
                return E_UNEXPECTED;
...
#pragma prefast(suppress: __WARNING_INCORRECT_ANNOTATION, "IAudioCaptureClient::GetBuffer SAL annotation implies a 1-byte buffer")
	...
	LOG(L"Silent: %u Discontinuity: %u and SilentAndDiscontinuity: %u", silentCount, discontinuityCount, silentAndDiscontinuityCount);
