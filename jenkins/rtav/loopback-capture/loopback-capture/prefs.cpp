...
void usage(LPCWSTR exe) {
    LOG(
		...
		L"    --quit-on-receive quits exe when receive first packet of sound data",
    ...
    switch (argc) {
        ...
        default:
            ...
            for (int i = 1; i < argc; i++) {
				...
				// --quit-on-receive
				if (0 == _wcsicmp(argv[i], L"--quit-on-receive")) {
					if (m_bQuit) {
						ERR(L"%s", L"Only one --quit-on-receive switch is allowed");
						hr = E_INVALIDARG;
						return;
					}
